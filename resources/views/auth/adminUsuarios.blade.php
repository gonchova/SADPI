<x-app-layout>
    <div class="flex flex-col max-h-screen">
        <div class="flex bg-white shadow-sm sm:rounded-lg mt-2 mb-2 justify-center ">
            <h2 class="text-gray-700 uppercase antialiased text-lg font-bold sm:text-sm">Usuarios</h2>
        </div>
        ...
        @if(session('status'))
            ...
            <form name="form-elimina-usuario" action="{{route('usuarios.eliminar', [ Session::get('idusuario') , 'true'])}}" method="POST">
                @method('DELETE')
                @csrf    
                <!-- Main modal -->
        ...
        <div class="flex-row pt-4 mx-1 sm:mx-2">
            <div class="bg-white shadow-sm sm:rounded-lg pb-2 mx-2">
                <div class="flex-full items-center justify-between pt-1">
                    
                    <form method="GET" action="{{route("usuarios.filtrar")}}" >
                        
                        <div class="flex-full inline-flex flex-wrap px-2">   
                            
                            {{-- <div  class="mx-2 w-48 shrink flex-wrap">
                                <x-label class=" sm:flex pt-3  text-xs sm:text-sm" for="rolesFiltro" :value="__('Roles')" />
                                <select name="rolesFiltro" id="rolesFiltro" class="text-xs sm:text-sm inline-flex items-center my-1 py-2 px-3 w-full h-auto text-white bg-gray-800 focus:outline-none focus:bg-purple-300 focus:text-black focus:ring-4 focus:ring-gray-200 font-medium rounded-lg  border-gray-600  " >
                                    <option value="" >Todas</option>
                                    @foreach ($especialidades as $esp)
                                        <option value= {{$esp->idespecialidad}} {{(old('rolesFiltro') == strtolower($esp->descripcion)) ? 'selected' : '' }}> {{$esp->descripcion}} </option>                      
                                    @endforeach
                                </select>
                            </div>                             --}}
                            
                            <div class="flex mx-2 w-48 shrink">
                                <x-label class="text-xs sm:text-sm sm:flex sm:pl-3 pt-3 " for="buscaUsuario" :value="__('Buscar:')" />    
                                <div class="flex flex-row">
                                    <input name = "buscaUsuario" id = "buscaUsuario" placeholder="Buscar usuario" class="text-xs sm:text-sm focus:outline-none hover:border-purple-300 focus:ring-2 focus:ring-purple-200 mt-2 bg-gray-200 rounded-full  px-3 py-1  font-semibold text-gray-700 mr-2 mb-2  sm:ml-2"> 
                            ...
                            @foreach($usuarios as $usr)
                                ...
                                <tr class="flex flex-col sm:table-row mb-2 sm:mb-0 max-w-screen mr-2">
                                    ...
                                    <td class="border-grey-light border hover:bg-gray-100 p-3 whitespace-nowrap">
                                        {{($usr->roles->descripcion);}}
                                    </td> 

                                    <td class="gap-1 border hover:bg-gray-100 p-1 flex-nowrap whitespace-nowrap ">
                                    
                                        {{-- <x-button class="px-2 py-2">
                                            <a href="{{route('actividades.editar', $usr->id )}}" class="sm:p-2">Editar</a>
                                        </x-button> --}}

                                         <!-- Modal toggle -->
                                        <x-button class="py-2 px-2 bg-red-600"  id="btnEliminar" name="btnEliminar" id-usr="{{$usr->id }}" data-modal-target="defaultModal" data-modal-toggle="defaultModal" type="button">
                                        ...
                                        <form name="form-elimina-usuario" action="{{route('usuarios.eliminar', ['IDUSR', 'false'])}}" method="POST">
                                            @method('DELETE')
                                            @csrf    
                                            <!-- Main modal -->
                                            <div id="defaultModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                                <div class="relative w-[50%] max-w-2xl max-h-full border-2 border-gray-500 rounded-lg">
                                                    ...
                                                    <div class="relative bg-white rounded-lg shadow">
                                                        ...
                                                        <div class="p-6 space-y-6">
                                                            <p class=" text-black">
                                                                Desea eliminar el usuario seleccionado junto con toda la actividad realizada?
                                                            </p>
                                                        </div>
                                                        <!-- Modal footer -->
                                                        <div class="flex flex-row p-6 space-x-2 border-t border-gray-200 rounded-b justify-center">
                                                            <x-button id-usr="{{$usr->id }}" name="ConfirmaEliminar" data-modal-hide="defaultModal" type="submit" class="px-2 py-2" >Aceptar</x-button>
            ...
            <div class="flex flex-row mx-4 pb-1">
                <a href="">{{ $usuarios->appends(['buscaUsuario' => request()->get('buscaUsuario') ])->links('vendor.pagination.tailwind') }}</a> 
            </div>
...
$(document).ready(function(){  
    ...
    $('[name=btnEliminar]').click(function(){
         ...
         var idusuario= $(this).attr('id-usr');
        console.log(idusuario);
         $("input[name=idusuario]").val(idusuario);
          
         var form = $('[name=form-elimina-usuario]');
         form.attr('action', form.attr('action').replace('IDUSR', idusuario));
         form.attr('action', form.attr('action').replace('false', 'true'));
